...
if [ $(grep -c "import /init.ck.rc" /tmp/ramdisk/init.rc) == 1 ]; then
	sed -i "/import \/init.ck.rc/d" /tmp/ramdisk/init.rc
fi

# Remove my modules from ramdisk
if [ -e /tmp/ramdisk/ck_modules ]; then
	rm -rf /tmp/ramdisk/ck_modules
fi
...
if [ -e /tmp/ramdisk/init.ck.rc ]; then
	rm /tmp/ramdisk/init.ck.rc
fi
