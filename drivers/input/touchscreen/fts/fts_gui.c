...
ssize_t fts_i2c_wr_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	memset(data, 0x00, ARRAY_SIZE(data) * sizeof(*data));
	memset(info->cmd_wr_result, 0x00, ARRAY_SIZE(info->cmd_wr_result) * sizeof(*info->cmd_wr_result));
...
ssize_t fts_i2c_read_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	byte_count_read = 0;
	memset(data, 0x00, ARRAY_SIZE(data) * sizeof(*data));
	memset(info->cmd_read_result, 0x00, ARRAY_SIZE(info->cmd_read_result) * sizeof(*info->cmd_read_result));
	sscanf(buf, "%x %x %x %x %x %x %x %x ", (data + 7), (data), (data + 1), (data + 2), (data + 3), (data + 4), (data + 5), (data + 6));
	byte_count = data[7];
...
ssize_t fts_i2c_write_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	int i ;
	memset(data, 0x00, ARRAY_SIZE(data) * sizeof(*data));
	memset(pAddress_i2c, 0x00, ARRAY_SIZE(pAddress_i2c) * sizeof(*pAddress_i2c));
	memset(info->cmd_write_result, 0x00, ARRAY_SIZE(info->cmd_write_result) * sizeof(*info->cmd_write_result));
